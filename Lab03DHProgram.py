...
class SecurePRNG:
    ...
    def generate(self, n_bytes):
        ...
        while len(output) < n_bytes:
            ...
            #    Hashing the state gives us 32 pseudorandom bytes of keystream.
            keystream_block = hashlib.sha256(self.state).digest()
            output += keystream_block

            # 2. Update state immediately after with a hash function (One-way progression).
            #    Prefixing 0x01 ensures the new state is
            #    distinct from the keystream block, providing rollback resistance.
            self.state = hashlib.sha256(b'\x01' + self.state).digest()
...
class Entity:
    ...
    def __init__(self, name):
        self.name = name
        self.private_key = secrets.randbelow(P - 2) + 2   # random in [2, P-1]
        self.public_key = pow(G, self.private_key, P)      # G^private mod P
...
class Mallory:
    def __init__(self):
        # Mallory generates her own DH key pair.
        # She will use this single key pair to impersonate Bob to Alice and
        # impersonate Alice to Bob, maintaining two independent PRNG sessions.
        self.private_key = secrets.randbelow(P - 2) + 2
        self.public_hex = hex(pow(G, self.private_key, P))

        # Mallory maintains TWO sessions
        # alice_prng is the shared secret between Mallory and Alice
        # bob_prng is the shared secret between Mallory and Bob
    ...
    def intercept(self, sender, recipient, payload):
        ...
        if isinstance(payload, str) and payload.startswith("0x"):
            ...
            if sender == "Alice":
                ...
                print(f"[MALLORY] Intercepted Alice's public key. Established session with Alice.")
            else:
                self.bob_prng = SecurePRNG(my_shared_secret)
                print(f"[MALLORY] Intercepted Bob's public key. Established session with Bob.")
        ...
        if isinstance(payload, bytes):
            ...
            plaintext_bytes = xor_crypt(payload, self.alice_prng)
            plaintext = plaintext_bytes.decode()
            print(f"[MALLORY] Decrypted Plaintext: '{plaintext}'")

            # Modify the plaintext message in some way
            modified_plaintext = plaintext.replace("9:00 PM", "3:00 AM")
            print(f"[MALLORY] Modified Plaintext:  '{modified_plaintext}'")

            # Then use the PRNG shared with bob to re-encrypt and return the message for Bob
            re_encrypted = xor_crypt(modified_plaintext.encode(), self.bob_prng)
            return re_encrypted
...
def main():
    ...
    message = b"We are meeting at 9:00 PM tonight. Don't be late!" # Put in your test message here
